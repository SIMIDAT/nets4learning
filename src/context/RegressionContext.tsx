...
import { DEFAULT_SELECTOR_DATASET_INDEX, DEFAULT_SELECTOR_MODEL_INDEX } from '@/CONSTANTS'
 ...
 *
 * @property {{data: _Types.DatasetProcessed_t[], index: number, dataset: 'select-dataset'}} datasets
 * @property {React.Dispatch<React.SetStateAction<{data: _Types.DatasetProcessed_t[], index: number, dataset: 'select-dataset'}>>} setDatasets
 *
...
type CustomRegressionContext_t = {
  ...
  setDatasets       : React.Dispatch<React.SetStateAction<{ data: _Types.DatasetProcessed_t[]; index: number; dataset: 'select-dataset'; }>>;
  params            : _Types.CustomParams_t;
  setParams         : React.Dispatch<React.SetStateAction<_Types.CustomParams_t>>;
  listModels        : { data: _Types.CustomModelGenerated_t[]; index: number; model: 'select-model'; };
  setListModels     : React.Dispatch<React.SetStateAction<{ data: _Types.CustomModelGenerated_t[]; index: number; model: 'select-model'; }>>;
  isTraining        : boolean;
...
export function RegressionProvider({ children }: { children: React.ReactNode }) {
   ...
   */
  const [datasets, setDatasets] = useState<{ data: _Types.DatasetProcessed_t[], index: number; dataset: 'select-dataset'; }>(
    {
   ...
   */
  const [params, setParams] = useState<_Types.CustomParams_t>(DEFAULT_PARAMS)

  /**
   * @type {ReturnType<typeof useState<{data: _Types.CustomModelGenerated_t[], index: number, model: 'select-model'}>>}
   */
  const [listModels, setListModels] = useState<{ data: _Types.CustomModelGenerated_t[], index: number, model: 'select-model' }>({
    data : [],
