    ...
    return <>
      ...
      <details>
        ...
        <ol>
          {Array
            .from({ length: 17 })
            .map((_, i) => <li key={i}><Trans i18nKey={prefix + 'details-input.list.' + i} /></li>)}
        </ol>
      </details>
      <details>
        ...
        <ol>
          {Array
            .from({ length: 4 })
            .map((_, i) => <li key={i}><Trans i18nKey={prefix + 'details-output.list.' + i} /></li>)}
        </ol>
      </details>
      <details>
        <summary>
...
`}
  ...
  async DATASETS(): Promise<_Types.DatasetProcessed_t[]> {
    const path_dataset = import.meta.env.VITE_PATH + '/models/00-tabular-classification/lymphography/'
  ...
  DEFAULT_LAYERS (): _Types.Layer_t[] {
    return [
      { _class: 'dense', units: 18, activation: 'sigmoid' },
      { _class: 'dense', units: 10, activation: 'relu'    },
      { _class: 'dense', units: 10, activation: 'relu'    },
      { _class: 'dense', units: 4,  activation: 'softmax' },
    ]
  }
