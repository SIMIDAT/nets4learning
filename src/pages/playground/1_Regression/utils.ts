 ...
 */
export function TRANSFORM_DATASET_PROCESSED_TO_STATE_PREDICTION (dataset_processed: _Types.DatasetProcessed_t, index_row = 0) {

  const {dataframe_original, dataframe_processed, data_processed} = dataset_processed  
  if (!data_processed) {
    console.error('data_processed is undefined')
    throw new Error('data_processed is undefined')
  }
  ...
  // @ts-expect-error append error ts
