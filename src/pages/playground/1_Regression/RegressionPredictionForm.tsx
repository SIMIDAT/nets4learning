 ...
 */
type RegressionPredictionFormProps_t = {
  generatedModel: _Types.CustomModelGenerated_t,
  dynamicObject?: any
}
/**
 * 
 ...
 * @returns 
 */
export default function RegressionPredictionForm (props: RegressionPredictionFormProps_t) {
  const { generatedModel } = props
  ...
  const getColor = (generatedModel: _Types.CustomModelGenerated_t, column_name: string) => {
  ...
  const isDisabled = (generatedModel: _Types.CustomModelGenerated_t, column_name: string) => {
    if (generatedModel.params_features.Y_target === column_name) {
      return true
    }
     if (generatedModel.params_features.X_features.has(column_name)) {
  ...
  const handleChange_EditInstanceEncoding = (column_name: string, new_value: any) => {
    if (generatedModel.dataset_processed.data_processed === undefined) {
      console.error('Error: dataset_processed.data_processed is undefined')
      return
    }
  ...
  const handleChange_EditInstance = (column_name: string, new_value: any) => {
    setPrediction((prevState) => {
      if (generatedModel.dataset_processed.data_processed === undefined) {
        console.error('Error: dataset_processed.data_processed is undefined')
        return prevState
