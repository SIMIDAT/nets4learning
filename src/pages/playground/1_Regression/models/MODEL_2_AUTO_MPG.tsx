...
`}
  ...
  async DATASETS (): Promise<_Types.DatasetProcessed_t[]> {
    const path_datasets = import.meta.env.VITE_PATH + '/datasets/01-regression/auto-mpg/'
  ...
  async MODELS (dataset: string = ''): Promise<_Types.CustomModel_t[]> {
    const path = import.meta.env.VITE_PATH + '/models/01-regression/auto-mpg'
  ...
  DEFAULT_LAYERS (): _Types.Layer_t[] {
    return [
      { is_disabled: false, units: 64, activation: 'relu', _class: 'dense' },
      { is_disabled: false, units: 64, activation: 'relu', _class: 'dense' },
      { is_disabled: true,  units: 1,  activation: 'linear',  _class: 'dense' }
    ]
  }
