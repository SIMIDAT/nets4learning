...
export default class MODEL_3_HOUSING_PRICES extends I_MODEL_REGRESSION {
  ...
  async DATASETS(): Promise<_Types.DatasetProcessed_t[]> {
    const path_datasets = import.meta.env.VITE_PATH + '/datasets/01-regression/housing-prices/'
  ...
  async MODELS(dataset: string = ''): Promise<_Types.CustomModel_t[]> {
    const path = import.meta.env.VITE_PATH + '/models/01-regression/housing-dataset'
  ...
  DEFAULT_LAYERS(dataset: string = ''): _Types.Layer_t[] {
    const models: { [key: string]: _Types.Layer_t[] } = {
      'california-housing-2024.csv': [
        { _class: 'dense', is_disabled: false, units: 20, activation: 'sigmoid' },
        { _class: 'dense', is_disabled: true, units: 1, activation: 'linear' },
      ],
      'boston-housing-2020.csv': [
        { _class: 'dense', is_disabled: false, units: 32, activation: 'sigmoid' },
        { _class: 'dense', is_disabled: false, units: 16, activation: 'sigmoid' },
        { _class: 'dense', is_disabled: true, units: 1, activation: 'linear' }
      ]
    }
    return models[dataset]
