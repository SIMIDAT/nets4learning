...
export default function ImageClassificationEditorHyperparameters(props: ImageClassificationEditorHyperparametersProps_t) {
  ...
  const handleChange_Loss = (e: React.ChangeEvent<HTMLSelectElement>) => {
    setIdLoss(e.target.value as IdLoss_t)
  }
  const handleChange_Optimization = (e: React.ChangeEvent<HTMLSelectElement>) => {
  ...
  const handleChange_Metrics = (e: React.ChangeEvent<HTMLSelectElement>, index: number) => {
    setIdMetricsList((prevState) => {
      const old_array = [...prevState]
      old_array[index] = e.target.value as IdLoss_t | IdMetric_t
      return old_array
    })
  }
  // endregion
    ...
    <Card className={'sticky-top joyride-step-7-editor-trainer'} style={{ zIndex: 10 }}>
      ...
      <Card.Body>
        ...
        <Form.Group className="mb-3" controlId="FormOptimizer">
          ...
          <Form.Select
            aria-label={t(prefix + 'optimizer-id-info')}
        ...
        <Accordion className={'mt-2'}>
          ...
          {idMetricsList.map((value, index) => {
            return <Accordion.Item key={index} eventKey={index.toString()}>
              <Accordion.Header>
                <Trans
                  i18nKey={prefix + 'metric-id-__index__'}
              ...
              <Accordion.Body>
                ...
                <Form.Group className="mb-3" controlId={`FormMetrics_${index}`}>
                  ...
                  <Form.Select
                    aria-label={t(prefix + 'metrics-id-info')}
                    value={value}
