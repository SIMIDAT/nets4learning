...
export default function TestPageEasy() {
  ...
  const handleClick_dataframe = () => {
    ...
    const df_vacio = new dfd.DataFrame([], { columns: df.columns, dtypes: df.dtypes })
    const new_df = df_vacio.append([[20, 40, 60, 'd']], [0])
    new_df.print()
  }
  ...
  const handleClick_TFJSMultiple = async () => {
    ...
    const p = (model.predict(tfjs.tensor2d([[5, 5], [6, 7]])) as tfjs.Tensor).dataSync()
    console.log({ predict_1: p })
  }

  const handleClick_TFJSMultiple_2 = async () => {
    // Generate a synthetic dataset
    const generateDataset = (numSamples: number) => {
    ...
    const prediction = model.predict(tfjs.tensor2d([to_predict_size_bedroom_distance])) as tfjs.Tensor
  ...
  const handleClick_TFJSMultiple_3 = async () => {
    ...
    if (!salary.data_processed) {
      console.error('Salary dataset not found')
      return
    }
    ...
    const linear_regression = await createRegressionCustomModel({
      ...
      idMetrics        : 'meanSquaredError' as any,
      layerList        : [
    ...
    if (!linear_regression) {
      console.error('linear_regression is null')
      return
    }
    ...
    console.log({
      predict_3: (model.predict(tfjs.tensor2d([years_n])) as tfjs.Tensor).dataSync()[0],
      target   : 13
    })
  }
  ...
  const handleClick_TFJSMultiple_4 = async () => {
    ...
    if (!auto.data_processed) {
      console.error('Auto MPG dataset not found')
      return
    }
    ...
    const linear_regression = await createRegressionCustomModel({
      ...
      idOptimizer      : 'sgd',
      idLoss           : 'losses-meanSquaredError',
      idMetrics        : ['meanSquaredError' as IdMetric_t] ,
      layerList        : [
        { units: 64, activation: 'sigmoid' },
        { units: 1, activation: 'linear' },
      ],
    ...
    console.log({
      predict_auto_1: (model.predict(tfjs.tensor2d([example_instance_n1])) as tfjs.Tensor).dataSync()[0],
      target_auto_1 : 17,
      predict_auto_2: (model.predict(tfjs.tensor2d([example_instance_n2])) as tfjs.Tensor).dataSync()[0],
      target_auto_2 : 24,
      predict_auto_3: (model.predict(tfjs.tensor2d([example_instance_n3])) as tfjs.Tensor).dataSync()[0],
      target_auto_3 : 13,
    })
  }
  ...
  const handleClick_TFJSMultiple_5 = async () => {
    ...
    console.log({ example_instance_df_n, example_instance_n, p: (model.predict(tfjs.tensor2d([example_instance_n])) as tfjs.Tensor).dataSync()[0] })

    const list = []
    for (let index = 0; index < 10; index++) {
      const element_n = X.values[index] as number[]
      const element_o = y.values[index]
      const element_p = (model.predict(tfjs.tensor2d([element_n])) as tfjs.Tensor).dataSync()[0]
