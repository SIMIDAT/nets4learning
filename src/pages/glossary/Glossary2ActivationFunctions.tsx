...
type ActivationFunction_t = {
  i18n_title_section: string
  layers: Array<{
    i18n_title: string
    img       : string | false
    latex     : string
...
export default function Glossary2ActivationFunctions() {
  ...
  const activationsFunctions: ActivationFunction_t[] = [
    // Linear Layers
    {
      ...
      layers            : [
        {
          i18n_title: "Linear",
          img       : IMGLinear,
          latex     : "$$ \\begin{split} R(z,m) = \\begin{Bmatrix} z*m \\end{Bmatrix} \\end{split} $$",
    ...
    // Non-linear Activations (weighted sum, non-linearity)
    {
      ...
      layers            : [
        ...
        },
        {
          ...
          img       : IMGSELU,
          latex:
            "$$ \\begin{split}  \\begin{Bmatrix} SELU(x) = scale * (max(0, x) + min(0, \\alpha * (e^x - 1))) \\\\ \\alpha = 1.6732632423543772848170429916717 \\\\ scale = 1.0507009873554804934193349852946. \\end{Bmatrix}\\end{split}$$",
        ...
        <Accordion.Item eventKey={"functions-activations"}>
          ...
          <Accordion.Body>
            ...
            <Table striped bordered hover responsive={true}>
              ...
              <tbody>
                {Object.entries(t("pages.glossary.activation-functions.table", { returnObjects: true })).map(
                  ([_function_key, function_info], index) => {
                      ...
                      <tr key={index}>
                        ...
                        <td>
                          <ol>
                            {characteristics && (
                              <>
                                {Object.entries(characteristics).map((p: [string, any], index_2: number) => {
                                  const [_sub_key, value] = p
